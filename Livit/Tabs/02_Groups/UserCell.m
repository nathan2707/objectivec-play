...
-(void)instaSetUp:(PFUser *)user_ :(NSInteger)rank{
    ...
    UIButton *denyButton = [[UIButton alloc]initWithFrame:CGRectMake(self.superview.superview.superview.frame.size.width-75,10,30,30)];
    denyButton.tag = rank;
    ...
    UIButton *acceptButton = [[UIButton alloc]initWithFrame:CGRectMake(self.superview.superview.superview.frame.size.width-40,10,30,30)];
...
-(void)setupView2: (PFUser *)user_{
    UIButton *acceptButton = [[UIButton alloc]initWithFrame:CGRectMake(self.superview.superview.superview.frame.size.width-40,20,30,30)];
    ...
    UIButton *wholeButton = [[UIButton alloc]initWithFrame:CGRectMake(0,0,self.superview.superview.superview.frame.size.width,self.frame.size.height)];
    [wholeButton addTarget:self action:@selector(actionBla:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:wholeButton];

}

-(void)setupView: (PFUser *)user_{
    UIButton *denyButton = [[UIButton alloc]initWithFrame:CGRectMake(self.superview.superview.superview.frame.size.width-90,20,30,30)];
...
-(void)actionBla:(UIButton*)sender{
    ...
    for (UIButton *butt in self.subviews) {
        if (butt != sender && [butt isKindOfClass:[UIButton class]]) {
            sender = butt;
            break;
        }
    }
